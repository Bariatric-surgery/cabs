...
def initialize_app():
    # Load the model
    model_preoperative_single = pkl.load(open(r'models/XGBoost_Optimized_Preoperative_Single_Output_delta_t.sav' , 'rb'))
    model_preoperative_multi = pkl.load(open(r'models/Random_Forest_Preoperative_Multi.sav' , 'rb'))
    model_postoperative_single = pkl.load(open(r'models/XGBoost_Postoperative_Single_Output_delta_t.sav' , 'rb'))
    model_postoperative_multi = pkl.load(open(r'models/Random_Forest_Postoperative_Multi.sav' , 'rb'))
    ...
    return model_preoperative_single , model_preoperative_multi, model_postoperative_single, model_postoperative_multi, data, bmi_columns, diferences_down, diferences_up
###############################################################################
# Section with the functions that the app will use
# Function that build the X input for the model single with preoperative information
def preprocess_input_case_1_single(sex , age_years , asa , charleson_comorbidity_index, depression,
                     prior_abdominal_surgery, diabetes_mellitus_preoperative, antidiabetic_drugs_preoperativ___1 , antidiabetic_drugs_preoperativ___2, antidiabetic_drugs_preoperativ___3 , antidiabetic_drugs_preoperativ___4,
                     osas_preoperaative, surgery, conversion_f, complication___7, bmi_pre):
    ...
    # Parser time_step
    information['delta_t'] = [float(1)]
    # Convert to a dataframe
    information = pd.DataFrame(data = information)
    
    return information
# Function that build the X input for the model multioutput with preperative information
def preprocess_input_case_1_multi(sex , age_years , asa , charleson_comorbidity_index, depression,
                     prior_abdominal_surgery, diabetes_mellitus_preoperative, antidiabetic_drugs_preoperativ___1 , antidiabetic_drugs_preoperativ___2, antidiabetic_drugs_preoperativ___3 , antidiabetic_drugs_preoperativ___4,
                     osas_preoperaative, surgery, conversion_f, complication___7, bmi_pre):
...
# Function that build the X input for the model single with postoperative information
def preprocess_input_case_2_multi(sex , age_years , asa , charleson_comorbidity_index, depression,
                            ...
                            bmi_12_months, bmi_18_months, bmi_2_years, bmi_3_years, bmi_4_years, bmi_5_years, no_complications):
    ...
    if no_complications == True:
        information['preventive_closure_of_mesenteric_defects'] = [int(1)]
    else:
...
# Function that build the X input for the model multi with postoperative information
def preprocess_input_case_2_single(sex , age_years , asa , charleson_comorbidity_index, depression,
                            prior_abdominal_surgery, diabetes_mellitus_preoperative, diabetes_mellitus_after_surgery,
                            antidiabetic_drugs_preoperativ___1 , antidiabetic_drugs_preoperativ___2, antidiabetic_drugs_preoperativ___3 , antidiabetic_drugs_preoperativ___4,
                            postoperative_hba1c_6_months , postoperative_hba1c_12_months , osas_preoperaative, osas_after_surgery,
                            surgery, conversion_f, surgery_time, preventive_closure_of_mesenteric_defects, complication___1,
...
# Initialize app
model_preoperative_single , model_preoperative_multi, model_postoperative_single, model_postoperative_multi, data, bmi_columns, diferences_down, diferences_up = initialize_app()
...
if selected == 'Case 1':
    ...
    bmi_pre = st.slider("BMI pre value:" , min_value = 35.0, max_value = 100.0, step = 0.1)
    st.subheader("Prediction result: ")
    # Parser user information
    user_input_1 = preprocess_input_case_1_multi(sex, age_years, asa, charleson_comorbidity_index, depression, prior_abdominal_surgery, diabetes_mellitus_preoperative, antidiabetic_drugs_preoperativ___1, antidiabetic_drugs_preoperativ___2, antidiabetic_drugs_preoperativ___3, antidiabetic_drugs_preoperativ___4, osas_preoperaative, surgery, conversion_f, complication___7, bmi_pre)
    user_input_2 = preprocess_input_case_1_single(sex, age_years, asa, charleson_comorbidity_index, depression, prior_abdominal_surgery, diabetes_mellitus_preoperative, antidiabetic_drugs_preoperativ___1, antidiabetic_drugs_preoperativ___2, antidiabetic_drugs_preoperativ___3, antidiabetic_drugs_preoperativ___4, osas_preoperaative, surgery, conversion_f, complication___7, bmi_pre)
    # Prediction
    predict_button = st.button('Predict')
    if predict_button:
        # Check that bmi pre is equal or greater than 35
        number_of_warnings = 0
        if user_input_1['bmi_pre'].values[0] < 35:
            st.warning('The value of BMI 6 pre is lower than 35, check the input', icon="⚠️")
            number_of_warnings += 1
        if number_of_warnings == 0:
            # Predict all BMI values at once using patients information and bmi pre, using multi output model
            bmi_columns = ['bmi_6_months','bmi_12_months','bmi_18_months','bmi_2years', 'bmi_3years', 'bmi_4years', 'bmi_5years']
            pred_multi = pd.DataFrame(model_preoperative_multi.predict(user_input_1.drop(columns = ['BMI(t)'])) , columns = bmi_columns).T
            pred_multi.columns = ['BMI Value']
            # Add bmi pre
            pred_multi = pd.concat([pd.DataFrame({'BMI Value' : [user_input_1['bmi_pre'].values[0]]}),
                                    pred_multi] , axis = 0)
            ...
            st.pyplot(fig)
            # Predict using single output model with delta t but using just delta_t = 1
            ...
            for i in range(len(bmi_columns)):
                if i == 0:
                    # The initial bmi t+1 is based on the last bmi real value
                    aux_prediction = pd.DataFrame(model_preoperative_single.predict(user_input_2.drop(columns = ['bmi_pre'])) , index = [bmi_columns[i]] , columns = ['BMI Value'])
                    ...
                    aux_bmi_time_step = pd.DataFrame({'delta_t' : [1]})
                    aux_x = pd.concat([user_input_2.drop(columns = ['bmi_pre' , 'BMI(t)' , 'delta_t']),
                                       aux_bmi_t,
                                       aux_bmi_time_step] , axis = 1)
                    aux_x = aux_x[user_input_2.drop(columns = ['bmi_pre']).columns.tolist()]
                    aux_prediction = pd.DataFrame(model_preoperative_single.predict(aux_x) , index = [bmi_columns[i]] , columns = ['BMI Value'])
                    pred_single = pd.concat([pred_single,
                                             aux_prediction] , axis = 0)
            # Add bmi pre
            pred_single = pd.concat([pd.DataFrame({'BMI Value' : [user_input_2['bmi_pre'].values[0]]}),
                                     pred_single] , axis = 0)
            # Add limits of the confident interval
            pred_single['Lower Difference'] = pd.DataFrame(diferences_down[0:] , index = pred_single.index.tolist())
            pred_single['Upper Difference'] = pd.DataFrame(diferences_up[0:] , index = pred_single.index.tolist())
            pred_single['Lower Bound'] = pred_single['BMI Value'] + pred_single['Lower Difference']
            pred_single['Upper Bound'] = pred_single['BMI Value'] + pred_single['Upper Difference']
            pred_single = pred_single.drop(columns = ['Lower Difference' , 'Upper Difference'])
            # Create the plot
            #st.header("Curve Plot of Evolution of BMI - Single Output Model delta_t = 1")
            y = pred_single['BMI Value'].values.tolist()
            x = ['PRE' ,'6M' , '12M' , '18M' , '2Y' , '3Y' , '4Y' , '5Y']
            fig, ax = plt.subplots()
            ax.plot(x[:2], y[:2] , color = 'blue' , label = 'Real BMI Values')
            ax.plot(x[1:], y[1:] , color = 'black' , label = 'Predicted BMI Values')
            ax.legend()
            ax.fill_between(
                x, pred_single['Lower Bound'], pred_single['Upper Bound'], color='gray', alpha=.15)
            ax.set_title('Evolution of BMI Value')
            ax.set_xlabel('BMI Time Step', 
                       fontweight ='bold')
            ax.set_ylabel('BMI',fontweight ='bold')
            fig.autofmt_xdate(rotation=45)
            #st.pyplot(fig)
            # Predict using single output model with delta t but using only bmi_pre and changing delta_t
            # Calculate the number of predictions we have to do
            pred_single = pd.DataFrame()
            # For loop to predict all future value susing single output model
            for i in range(len(bmi_columns)):
                if i == 0:
                    ...
                    # The future bmis will be predicted based on the previous prediction
                    aux_bmi_time_step = pd.DataFrame({'delta_t' : [i+1]})
                    aux_x = pd.concat([user_input_2.drop(columns = ['bmi_pre' , 'delta_t']),
                                       aux_bmi_time_step] , axis = 1)
                    aux_x = aux_x[user_input_2.drop(columns = ['bmi_pre']).columns.tolist()]
                    aux_prediction = pd.DataFrame(model_preoperative_single.predict(aux_x) , index = [bmi_columns[i]] , columns = ['BMI Value'])
                    pred_single = pd.concat([pred_single,
            ...
            pred_single = pred_single.drop(columns = ['Lower Difference' , 'Upper Difference'])
            # Create the plot
            #st.header("Curve Plot of Evolution of BMI - Single Output Model BMI(t) = BMI pre")
            y = pred_single['BMI Value'].values.tolist()
            x = ['PRE' ,'6M' , '12M' , '18M' , '2Y' , '3Y' , '4Y' , '5Y']
            fig, ax = plt.subplots()
            ax.plot(x[:2], y[:2] , color = 'blue' , label = 'Real BMI Values')
...
if selected == 'Case 2':
    ...
    st.markdown("""
    1. Enter clinical parameters of patient on the left side bar.
    2. Choose on the check box if the patient has complications
    3. Choose the real BMI values you have.
    4. Press the "Predict" button and wait for the result.
    """)
    # Complications checkbox
    complications_checkbox = st.checkbox('No complications')
    ...
    conversion_f = st.sidebar.selectbox('Conversion from gastric sleeve to gastric bypass', ('Yes', 'No', 'Unknown'))
    surgery_time = st.sidebar.slider("Surgery time (min):", min_value = 25, max_value = 352,step = 1)
    # Use checkbox to parser the information of patienst complication
    if not complications_checkbox:
        preventive_closure_of_mesenteric_defects = st.sidebar.selectbox('Preventive Closure of Mesenteric Defects type', (1, 2, 3))
        complication___1 = st.sidebar.selectbox('Anastomotic leakage', ('Yes' , 'No'))
        complication___2 = st.sidebar.selectbox('Gastric leakage', ('Yes' , 'No'))
        complication___3 = st.sidebar.selectbox('Intussusception after Roux-en-Y gastric bypass', ('Yes' , 'No'))
        ...
        preventive_closure_of_mesenteric_defects = 1
        complication___1 = 'No'
        complication___2 = 'No'
        complication___3 = 'No'
        complication___4 = 'No'
        ...
        complication___8 = 'No'
        complication___9 = 'No'
        the_clavien_dindo_classification = 'No complication'
    ...
    if multi_select_list[0] in multiselect_bmi_future:
        bmi_6_months = st.slider("BMI 6 months value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
    ...
    if multi_select_list[1] in multiselect_bmi_future:
        bmi_12_months = st.slider("BMI 12 months value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
    ...
    if multi_select_list[2] in multiselect_bmi_future:
        bmi_18_months = st.slider("BMI 18 months value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
        multi_select_list_2.append('bmi_18_months')
    if multi_select_list[2] not in multiselect_bmi_future:
        bmi_18_months = -1
    if multi_select_list[3] in multiselect_bmi_future:
        bmi_2_years = st.slider("BMI 2 years value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
    ...
    if multi_select_list[4] in multiselect_bmi_future:
        bmi_3_years = st.slider("BMI 3 years value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
    ...
    if multi_select_list[5] in multiselect_bmi_future:
        bmi_4_years = st.slider("BMI 4 years value:" , min_value = 18.0, max_value = 100.0, step = 0.1)
    ...
    if predict_button:
        ...
        if user_input_1['bmi_6_months'].values[0] == 0:
            st.warning('The value of BMI 6 months is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        if ((user_input_1['bmi_12_months'].values[0] != 0) or (user_input_1['bmi_18_months'].values[0] != 0) or (user_input_1['bmi_2years'].values[0] != 0) or (user_input_1['bmi_3years'].values[0] != 0) or (user_input_1['bmi_4years'].values[0] != 0) or (user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_6_months'].values[0] == 0:
            st.warning('The value of BMI 6 months is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        # Check for bmi 12 months
        if ((user_input_1['bmi_18_months'].values[0] != 0) or (user_input_1['bmi_2years'].values[0] != 0) or (user_input_1['bmi_3years'].values[0] != 0) or (user_input_1['bmi_4years'].values[0] != 0) or (user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_12_months'].values[0] == 0:
            st.warning('The value of BMI 12 months is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        # Check for bmi 18 months
        if ((user_input_1['bmi_2years'].values[0] != 0) or (user_input_1['bmi_3years'].values[0] != 0) or (user_input_1['bmi_4years'].values[0] != 0) or (user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_18_months'].values[0] == 0:
            st.warning('The value of BMI 18 months is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        # Check for bmi 2 years
        if ((user_input_1['bmi_3years'].values[0] != 0) or (user_input_1['bmi_4years'].values[0] != 0) or (user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_2years'].values[0] == 0:
            st.warning('The value of BMI 2 years is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        # Check for bmi 3 years
        if ((user_input_1['bmi_4years'].values[0] != 0) or (user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_3years'].values[0] == 0:
            st.warning('The value of BMI 3 years is 0, check the input', icon="⚠️")
            number_of_warnings += 1
        # Check for bmi 4 years
        if ((user_input_1['bmi_5years'].values[0] != 0)) and user_input_1['bmi_4years'].values[0] == 0:
        ...
        if number_of_warnings == 0: # Perform prediction only if there is no warning
            # Predict the single BMI(t+1) value using delta_t = 1
            ...
            if user_input_2['bmi_6_months'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_6_months']
            elif user_input_2['bmi_12_months'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_12_months']
            elif user_input_2['bmi_18_months'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_18_months']
            elif user_input_2['bmi_2years'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_2years']
            elif user_input_2['bmi_3years'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_3years']
            elif user_input_2['bmi_4years'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_4years']
            elif user_input_2['bmi_5years'].values[0] == user_input_2['BMI(t)'].values[0]:
                last_bmi_value = user_input_2['bmi_5years']
            ...
            for i in range(len(bmi_columns) - iterations - 1):
                if i == 0:
                    # The initial bmi t+1 is based on the last bmi real value
                    aux_prediction = pd.DataFrame(model_postoperative_single.predict(user_input_2.drop(columns = columns_to_drop)) , index = [bmi_columns[iterations + i + 1]] , columns = ['BMI Value'])
                    pred_single = pd.concat([pred_single,
                                             aux_prediction] , axis = 0)
                else:
                    # The future bmis will be predicted based on the previous prediction
                    aux_bmi_t = pd.DataFrame({'BMI(t)' : [pred_single.iloc[-1].values[0]]})
                    aux_bmi_time_step = pd.DataFrame({'delta_t' : [1]})
                    aux_x = pd.concat([user_input_2.drop(columns = columns_to_drop + ['BMI(t)' , 'delta_t']),
                                       aux_bmi_t,
                                       aux_bmi_time_step] , axis = 1)
                    aux_prediction = pd.DataFrame(model_postoperative_single.predict(aux_x) , index = [bmi_columns[iterations + i + 1]] , columns = ['BMI Value'])
                    pred_single = pd.concat([pred_single,
                                             aux_prediction] , axis = 0)
            # Add the real values to the predictions
            real_bmi_columns = bmi_columns
            real_bmi = user_input_2[real_bmi_columns].T
            ...
            fig.autofmt_xdate(rotation=45)
            #st.pyplot(fig)
            # Predict the single BMI(t+1) value using BMI(t) = last bmi real value
            columns_to_drop = ['bmi_pre' , 'bmi_6_months','bmi_12_months','bmi_18_months','bmi_2years', 'bmi_3years', 'bmi_4years', 'bmi_5years']
            #pred = model_single.predict(user_input.drop(columns = columns_to_drop))
            #st.header("BMI(t+1):")
            #st.subheader(str(round(pred[0] , 2)) + ' (' + str(round(pred[0] + np.mean(diferences_down) , 2)) + ' ; ' + str(round(pred[0] + np.mean(diferences_up) , 2)) + ')')
            ...
            for i in range(len(bmi_columns) - iterations - 1):
                if i == 0:
                    ...
                    # The future bmis will be predicted based on the previous prediction
                    aux_bmi_time_step = pd.DataFrame({'delta_t' : [i + 1]})
                    aux_x = pd.concat([user_input_2.drop(columns = columns_to_drop + ['delta_t']),
                                       aux_bmi_time_step] , axis = 1)
                    aux_prediction = pd.DataFrame(model_postoperative_single.predict(aux_x) , index = [bmi_columns[iterations + i + 1]] , columns = ['BMI Value'])
                    pred_single = pd.concat([pred_single,
                                             aux_prediction] , axis = 0)
                    print('i post:' , i , aux_bmi_time_step)
            # Add the real values to the predictions
            real_bmi_columns = bmi_columns
            real_bmi = user_input_2[real_bmi_columns].T
            real_bmi.columns = ['BMI Value']
            ...
            real_bmi = real_bmi.drop(columns = ['Lower Difference' , 'Upper Difference'])
            # Create the plot
            #st.header("Curve Plot of Evolution of BMI - Single Output Model BMI(t) = last bmi value")
            y = real_bmi['BMI Value'].values.tolist()
            x = real_bmi.index.tolist()
            fig, ax = plt.subplots()
            ax.plot(x[: len(real_bmi_columns)], y[: len(real_bmi_columns)] , color = 'blue' , label = 'Real BMI Values')
            ...
            pred_multi = pd.DataFrame(model_postoperative_multi.predict(user_input_1.drop(columns = bmi_columns_2 + ['BMI(t)'])) , columns = bmi_columns_2).T
            pred_multi.columns = ['BMI Value']
            pred_multi = pd.concat([pd.DataFrame({'BMI Value' : [user_input_1['bmi_pre'].values[0]]} , index = ['bmi_pre']),
                                    pred_multi] , axis = 0) 
            # Add the real values
            real_bmi_columns = bmi_columns
            real_bmi = user_input_1[real_bmi_columns].T
            real_bmi.columns = ['BMI Value']
            real_bmi = real_bmi[real_bmi['BMI Value'] != -1]
            real_bmi_columns = real_bmi.index.tolist()
